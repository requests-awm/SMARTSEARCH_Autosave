...
export async function runPipeline(taskGid, emit) {
  if (activeRuns.has(taskGid)) {
    emit({ done: true, ok: false, error: 'This task is already being processed' });
    return;
  }
  activeRuns.add(taskGid);
  try {
    await runPipelineInner(taskGid, emit);
  } finally {
    activeRuns.delete(taskGid);
  }
}

async function runPipelineInner(taskGid, emit) {
  ...
  try {
    await step('load', 'Load task & verify checks', async () => {
      ...
      if (s.isRefer) {
        throw new Error(
          s.inReferSection
            ? 'Task is in the SmartSearch "Refer" section — Refer results are not processed automatically'
            : `Expiry date ${s.expiryDate} is the "Refer" dummy value — Refer results are not processed automatically`
        );
      }
