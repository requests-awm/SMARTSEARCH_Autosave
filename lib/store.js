...
import { mirrorRecord } from './supabase.js';
...
export function recordsWithReminders() {
  ...
  return load()
    ...
    .map((r) => {
      ...
      let status;
      if (todayIso > r.expiryDate) status = 'expired';
      else if (todayIso >= dueDate) status = 'due';
      else status = 'upcoming';
      if (r.reminderDismissed && (status === 'due' || status === 'expired')) status = 'dismissed';
      return { ...r, reminder: { dueDate, status, daysToExpiry: daysBetween(todayIso, r.expiryDate) } };
    })
